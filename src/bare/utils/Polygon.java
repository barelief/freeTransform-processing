...
public class Polygon {
	...
	public final int amount = 4; // how many edges in polygon
	int selectedPoint = -1; // currently mouse selected point
							// http://i.imgur.com/Iq2ZZhT.jpg
	...
	PVector X, Y, Z, Q; // lines, made of line centre points, intersection
						// points, aka lines which create PVector anchor
						// (http://i.imgur.com/Vw7ZXAf.png)
	Line XZ, QY; // intersection Lines http://i.imgur.com/Vw7ZXAf.png

	Line diagonal; // line connecting current selected/focused point with
					// opposite point -- used for SCALE_PORPORTIONALLY_POINT
					// state
	Line leftOppositeLine, rightOppositeLine; // used for POINTS_SCALE_FREE to
												// find offsetX on those lines
												// (?)
	...
	// point where point lines intersect; http://i.imgur.com/uoUt5Lp.jpg
	PVector center;
	...
	PVector offset; // offset to move point offsetP made by moving user point P
					// (check offsetPooint.pde)
	PVector beginOffset; // check check offsetPooint.pde NEED MORE explanation

	// boolean pointLockedToMouse = false; // if one of four points is dragged,
	// then ignore other points by locking the selected one
	boolean polygonLockedToMouse = false;
	boolean rotationLockedToMouse = false;
	boolean mouseLockedToLine = false;

	// on which line side is the mouse pointer (needed for mirror )
	boolean isOnThisSideOfLine = true;

	boolean dragLock = false; // locks all detections when started dragging any
								// object
	State state = State.NONE; // mouse interaction states, check State.java

	int pointMode = 0; // 0 - POINT, 1 - SCALE_FREE_POINT, 2 -
						// SCALE_PORPORTIONALLY_POINT
	// 0 - SCALE_PROPORTIONALLY_LINE, 1 - DRAG_FREE_LINE
	int lineMode = 0;

	int rotateMode = 0; // 1 - rotate with 45degr step

	JSONArray values; // coordinates for polygons points loaded from external
						// file
	...
	void createPoints() {
		for (int j = 0; j < amount; j++) {
			point[j] = new Point(myParent, 0, 0, j, this);
			// println(i+":
			// "+values.getJSONObject(i).getInt("x")+","+values.getJSONObject(i).getInt("y"));
		}
	}

	// reset quad to simple quad
	void reset(int size) {
	...
	void repositionQuad(PVector newPosition) {
	...
	/**
	 * this function sets the interaction state based on mouse position listed
	 * by focus priority, i.e. first focus on point, then on line, then on area,
	 * then on rotate
	 * 
	 */
	void fucusLocks() {
	...
	void setupLines() {
		...
		diagonal = new Line(myParent, new PVector(), new PVector());
		leftOppositeLine = new Line(myParent, new PVector(), new PVector());
		rightOppositeLine = new Line(myParent, new PVector(), new PVector());
		leftNeighborLine = new Line(myParent, new PVector(), new PVector());
		rightNeighborLine = new Line(myParent, new PVector(), new PVector());
		oppositeLine = new Line(myParent, new PVector(), new PVector());
	}
	...
	void setupLineIntersections() {
		XZ = new Line(myParent, new PVector(), new PVector());
		QY = new Line(myParent, new PVector(), new PVector());
		anchor = new PVector();
	}
	...
	// update two global points based on currently dragged line (by using global
	// var - selectedLine)
	...
	// find neighbor points based on a source line id, check drawing:
	...
	void update() {
		...
		// find anchor point (aka intersection of lines from the centres of main
		// lines)
	...
	void render() {
		...
		// not sure if it's ok to put it inside render() method
		if (!dragLock)
			fucusLocks();
	...
	void displayOppositeObject() {
		...
		// avoid out of bounds when slected object is none
	...
	// like invisible dot product X point, neighbors, intersections, states,
	// point focus etc press D to see
	...
	// shows neighbors and opposite points to currently selected point (debug
	// purpose)
	...
	void displayLineBetweenCurrentAndOppositePoints() {
		myParent.pushStyle();
		myParent.stroke(255, 255, 0);
		myParent.line(diagonal.start.x, diagonal.start.y, diagonal.end.x, diagonal.end.y);
		myParent.text("X", diagonal.X.x, diagonal.X.y);
		// line(0, 0, diagonal.offsetX.x, diagonal.offsetX.y);
		myParent.text("beginX", diagonal.beginX.x, diagonal.beginX.y);
	...
	void release() {
		// System.out.println("[notice ] releasing locks... ");
		for (int i = 0; i < amount; i++) {
	...
	// Drags the selected point while mouse is being dragged and state is
	// DRAG_FREE_POINT
	void dragPoint() {
	...
	void setupDragPoint() {
		...
		// updateGlobalLines() updates all but selectedLine, thats why when
		// dragging points selectedLine none of existing lines
		selectedLine = -1;
	}
	...
	// this function is executed once when mouse was pressed in the
	// DRAG_FREE_LINE state
	// initial offset vector positions (beginOffsetP, resetLockPoints()) are set
	void setupDragLines() {
	...
	// this function is executed while mouse is being dragged and the state is
	// DRAG_FREE_LINE
	void dragLines() {
	...
	void lineScaleProportional() {
		...
		// temporary vector XP http://i.imgur.com/bKosZNr.jpg
		PVector XP = new PVector();
		...
		// right point neighbor id is alwas the same as line id, check drawing:
		// http://i.imgur.com/iyZVj78.jpg
		// check int[] neighborPointsFromLine(int sourceLineId)
		point[neighborPointsFromLine(selectedLine)[1]].position.set(XP); // right
																			// neighbor
																			// point
																			// from
																			// selected
		...
		point[neighborPointsFromLine(selectedLine)[0]].position.set(XP); // left
																			// neighbor
																			// point
																			// from
																			// selected
	...
	void setupScalePointProportionally() {
		...
		diagonal = new Line(myParent, point[neighbor(selectedPoint)[2]].position, point[selectedPoint].position);
		diagonal.resetLockPoints(P);
		diagonal.beginOffsetX.set(point[selectedPoint].position); // lock the
																	// beginning
																	// of the
																	// vector to
																	// be
																	// transformed
		diagonal.beginStart.set(diagonal.X); // lock the beginning position of
												// the offset vector
	}
	...
	void scalePointProportionally() {
		...
		// offsetP = PVector.sub(P, beginOffsetP); // get the offset (because
		// mouse isnt' exatly at the pressed point, there a little offset)
		offset = PVector.sub(diagonal.X, diagonal.beginStart); // calculate the
																// offset made
																// by mouseDrag
																// -- subtract
																// beginOffset
																// from P
		diagonal.offsetX = PVector.add(diagonal.beginOffsetX, offset); // reposition
																		// point
																		// A
																		// based
																		// the
	...
	// This function is triggered once the mouse is pressed and focused on point
	// while holding SHIFT)
	...
	// This function is called whie dragging the selected point at
	// SCALE_PORPORTIONALLY_POINT state,
	// it "scales" the whole corner proportionally (aka according to aspect of
	// the polygon)
	...
	// This function is runnig while mouseWheel is triggered or +/- keys are
	// pressed
	...
	// This function rotates polygon (aka all points) around anchor point, check
	// rotate() inside Point class
	...
	// points is an array of polygon points; input is point to check if its
	// iside polygon (aka mouse)
